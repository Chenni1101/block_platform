        ...
        <a-card class="market-card market-control" :bordered="false">
          ...
          <div class="market-field">
            ...
            <a-select v-model:value="selectedAssetName" placeholder="请选择藏品" style="width: 100%">
              <a-select-option v-for="item in analysisTargets" :key="item.id" :value="item.name">
                {{ item.name }}
              </a-select-option>
            </a-select>
          </div>
...
const analysisTargets = computed(() => {
  const certified = exhibits.value.filter((item) => item.certified)
  return certified.length ? certified : exhibits.value
})
const selectedAssetName = ref(analysisTargets.value[0]?.name || exhibits.value[0]?.name || '')
